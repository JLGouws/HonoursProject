...
bool TrackerKCFImpl::initImpl( const Mat& image, const Rect2d& boundingBox ) {
  Rect roi(boundingBox.x, boundingBox.y, boundingBox.width , boundingBox.height);
  initO(image, roi);
  isInit = true;
...
bool TrackerKCFImpl::updateImpl( const Mat& image, Rect2d& boundingBox ) {
  double response;
  track(image, &response);
  BBox_c bb = getBBox();
  boundingBox.x = bb.cx - bb.w / 2.;
  boundingBox.y = bb.cy - bb.h / 2.;
  boundingBox.width = bb.w;
  boundingBox.height = bb.h;
...
void TrackerKCFImpl::initO(const cv::Mat &img, const cv::Rect & bbox)
{
...
void TrackerKCFImpl::initO(cv::Mat &img, const cv::Rect & bbox)
{
    //check boundary, enforce min size
    double x1 = bbox.x, x2 = bbox.x + bbox.width, y1 = bbox.y, y2 = bbox.y + bbox.height;
    if (x1 < 0) x1 = 0.;
...
void TrackerKCFImpl::setTrackerPose(BBox_c &bbox, cv::Mat & img)
{
    initO(img, bbox.get_rect());
}

void TrackerKCFImpl::updateTrackerPosition(BBox_c &bbox)
{
    if (p_resize_image) {
        BBox_c tmp = bbox;
        tmp.scale(0.5);
...
void TrackerKCFImpl::track(const cv::Mat &img, double *maxResponse)
{
    cv::Mat input_gray, input_rgb = img.clone();
    if (img.channels() == 3){
        cv::cvtColor(img, input_gray, CV_BGR2GRAY);
...
void TrackerKCFImpl::track(const cv::Mat &img)
{
    cv::Mat input_gray, input_rgb = img.clone();
    if (img.channels() == 3){
        cv::cvtColor(img, input_gray, CV_BGR2GRAY);
...
void TrackerKCFImpl::track(cv::Mat &img)
{
    cv::Mat input_gray, input_rgb = img.clone();
    if (img.channels() == 3){
        cv::cvtColor(img, input_gray, CV_BGR2GRAY);
...
std::vector<cv::Mat> TrackerKCFImpl::get_features(cv::Mat & input_rgb, cv::Mat & input_gray, int cx, int cy, int size_x, int size_y, double scale)
{
...
cv::Mat TrackerKCFImpl::gaussian_shaped_labels(double sigma, int dim1, int dim2)
{
...
cv::Mat TrackerKCFImpl::circshift(const cv::Mat &patch, int x_rot, int y_rot)
{
...
ComplexMat TrackerKCFImpl::fft2(const cv::Mat &input)
{
...
ComplexMat TrackerKCFImpl::fft2(const std::vector<cv::Mat> &input, const cv::Mat &cos_window)
{
...
cv::Mat TrackerKCFImpl::ifft2(const ComplexMat &inputf)
{
...
cv::Mat TrackerKCFImpl::cosine_window_function(int dim1, int dim2)
{
...
cv::Mat TrackerKCFImpl::get_subwindow(const cv::Mat &input, int cx, int cy, int width, int height)
{
    cv::Mat patch;
...
ComplexMat TrackerKCFImpl::gaussian_correlation(const ComplexMat &xf, const ComplexMat &yf, double sigma, bool auto_correlation)
{
...
cv::Point2f TrackerKCFImpl::sub_pixel_peak(cv::Point & max_loc, cv::Mat & response)
{
...
double TrackerKCFImpl::sub_grid_scale(std::vector<double> & responses, int index)
{
    cv::Mat A, fval;
